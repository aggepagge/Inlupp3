...
namespace Interaction.View.Particles
{
    class Explotion
    {
        ...
        private static float TOTAL_END_TIME = 0.3f;
        ...
        {
            delayTimeSeconds -= elapsedGameTime;

            //Kollar om animeringen ska uppdateras
            if (!stopExplotion && delayTimeSeconds <= 0.0f)
            {
        ...
        internal void Draw(SpriteBatch spriteBatch, Camera camera, Texture2D texture)
        {
            if (!stopExplotion)
            {
                ...
                spriteBatch.Draw(texture, frameRect, explotionRect, new Color(255, 255, 255, 105), rotation, new Vector2((texture.Width / numFramesX) / 2, (texture.Height / numFramesY) / 2), SpriteEffects.None, 0);
            }
        }
    }
}
